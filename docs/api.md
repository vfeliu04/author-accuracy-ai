...
| Closing text the reference scan sends to the model | 60,000 characters (`REFERENCE_MAX_CHARS`), from the last reference-list heading forward — or from the earliest heading of a run of headings at most 12,000 characters apart (`HEADING_RUN_GAP`): a multi-page bibliography repeats its heading on each page — else the last 60,000 of the document | In `POST /api/references/scan`; the model never sees more |
...
`text_source` says where the text came from: `heading` (from the reference-list heading, as above), `tail` (no heading; the document's end) or `none` (no text at all; `references` is then empty). At most 150 references are returned (`MAX_REFERENCES`); a longer model answer is cut, with a warning in the server log.
