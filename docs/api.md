...
| Time and memory to read the report PDF in a scan | 60 seconds of wall clock (`extract_timeout_seconds`, the budget a fetched web page gets), in a separate process stopped at the deadline, with a CPU limit of that budget plus 5 seconds, a 1 GiB address-space cap (`READER_ADDRESS_SPACE_BYTES`) as a backstop where the platform enforces one and it leaves the watchdog below its headroom over what the process already maps (Linux, read from `/proc/self/status`; macOS refuses the call), and on every platform a watchdog thread that ends the process once its peak resident size passes 768 MiB (`READER_MEMORY_BYTES`, read every quarter second). Before pypdf flattens the page tree the process walks it itself and refuses a declared page count over 10,000 (`PAGE_TREE_CEILING`), a walk of more than 10,000 nodes, or a node reached twice. What the process sends back is bounded too: each page's text is cut at 130,000 characters (`PAGE_TEXT_MAX_CHARS`), and past 8,000,000 characters in all (`READER_MAX_TEXT_CHARS`) the earlier pages are not read — the last pages are what come back, with a warning in the log | In `POST /api/references/scan` → 400 `could not read the PDF: it was too costly to read (…)` or `could not read the PDF: <refusal>` |
| Result a reader process may report | 64 MiB (`RESULT_MAX_BYTES` in `web.py`), refused on the frame's length header before a byte of its body is read | In `POST /api/references/scan` → 400 `could not read the PDF: it was too costly to read (the reader's result was too large)`; in the ingest step's page read → run `FAILED` |
...
| References returned by a scan | 300 (`MAX_REFERENCES`); a longer model answer is cut in code after every part has answered (the cut saves no model call), with a warning in the log, and the rows cut are counted in `limits.references_dropped`. Each `label` is at most 40 characters (`LABEL_MAX_CHARS`), each `title` at most 500 (`TITLE_MAX_CHARS`), `authors` at most 50 names (`MAX_AUTHORS`) of at most 200 characters each (`AUTHOR_MAX_CHARS`), all cut in code after parsing; a `url` over 2048 (`URL_FIELD_MAX_CHARS`, the link gate's own length) or a `doi` over 300 (`DOI_FIELD_MAX_CHARS`) is dropped from its row (`null`, with a warning in the log), never cut — a cut address of the gate's own length would pass the gate as a link the page never printed. A part's answer is held against the text it read: fewer objects than half the lines shaped like an entry's start (`entry_starts`, with at least 4 such lines, `ENTRY_STARTS_FLOOR`), or any string field over 1,000 characters before the cuts (`DEGENERATE_FIELD_CHARS`: entries merged into one object's text, or a field the model looped on; the longest real entry measured is 759), is asked for once more with the same prompt, and the answer that passes the check is kept (else the one with more references); an answer still short sets `limits.possibly_incomplete` | In `POST /api/references/scan` |
| Unpaywall lookups per scan | One per distinct printed DOI (a DOI several entries print is asked about once), four at a time, 10-second timeout and two retries each; the first registry failure ends the lookup, and so does a 45-second deadline on the lookup phase as a whole (`LOOKUP_DEADLINE_SECONDS`). A printed DOI over 256 characters (`DOI_MAX_CHARS`) is not looked up | In `POST /api/references/scan` → `lookup.status` `unavailable`, still 200 |
| Reference scans at once | 2 (`SCAN_CONCURRENCY` in `api.py`); a scan holds its slot from after the file checks to its answer, and a scan the dialog abandons holds it until it finishes | In `POST /api/references/scan` → 429 `a reference scan is already running — try again in a moment` |
...
| 429 | Reference scan | `{"detail": "a reference scan is already running — try again in a moment"}` — `SCAN_CONCURRENCY` (2) scans are already running; nothing was read or asked, and the request can be repeated once one finishes |
...
| 502 | Reference scan | `{"detail": "the model's answer could not be read — try the scan again"}` — a part of the reference list was answered with text that is not the JSON the schema asked for (live, a string the model looped on until the token limit), on the call and on its one retry; nothing was stored, and the request can simply be repeated |
| 500 | Reference scan, chat | The model call failed (a provider outage, or no parseable answer), or — the scan only — the process that reads the PDF failed on its own (an import error, a bug: `the report PDF could not be read: the reader process exited without a result`, its exit status in the log) rather than being stopped by a bound; nothing was stored, and the request can simply be repeated |
...
**The scan stores nothing.** It creates no run, upload or job row (the handler has no database dependency, so it cannot), writes no file under `uploads_dir` (the uploaded part is read in place), never fetches a cited work, and caches nothing — a second scan of the same file reads it and asks the model again. Only the PDF's closing pages are read: at most the last 600 pages, with pypdf rather than the ingest pipeline's layout parser, so the answer arrives while the user is still choosing sources. The read runs in a separate process under the bounds a fetched web page gets — `extract_timeout_seconds` (60) of wall clock, a CPU limit, a watcher that ends it with the server — plus, as a backstop, a 1 GiB address-space cap (`READER_ADDRESS_SPACE_BYTES`) where the platform enforces one and the cap leaves the watchdog below its 768 MiB of headroom over what the process already maps — `RLIMIT_AS` counts every mapping, and a Linux child maps hundreds of MB before pypdf runs, so a fixed cap could fall on a legitimate report before the watchdog did (Linux, read from `/proc/self/status`; macOS refuses the call) and, on every platform, a watchdog thread inside the process that ends it once its peak resident size passes 768 MiB (`READER_MEMORY_BYTES`, read every quarter second: the bound on what pypdf's FlateDecode can inflate where the address-space cap is not one — on macOS a 1.17 MB file inflating to 1.2 GB was otherwise read whole, at 1.27 GB resident, inside the deadline), because pypdf flattens the whole page tree before a page can be taken from its end, walking it with no visited set, so a file of a few hundred bytes whose nodes each list one child twice would yield 2^depth pages. Before pypdf flattens anything the process walks the tree itself and refuses a declared page count over 10,000 (`PAGE_TREE_CEILING`), a walk of more than 10,000 nodes, or a node reached twice (a shared child, a cycle), so the flatten it then permits costs at most that many nodes whatever the file declares. An encrypted file (opened with an empty user password, as readers do) is paged by its real page list rather than the trailer's declared page count, so a stale count neither drops the closing pages nor fails the read; and a glyph a broken font maps to an unpaired surrogate becomes `?`, so the text can be sent to the model. The process also bounds what it sends back, because the bounds above cap what pypdf may cost, not what it may return: a ToUnicode map that gives one glyph code a 30,000-character destination turned a 1.3 KB file into 240 MB of page text in a second, inside every budget, which the server then held three times over for 30 seconds of the request thread. Each page's text is cut at 130,000 characters (`PAGE_TEXT_MAX_CHARS`, twice the closing text the model reads, so a real page still opens a full window), and past 8,000,000 characters in all (`READER_MAX_TEXT_CHARS`) the earlier pages are not read — the pages are read from the end, so the last ones are what come back, with a warning in the log. The server refuses a result longer than 64 MiB (`RESULT_MAX_BYTES`) on its length header, before reading a byte of it, as one more bound. A read stopped by a budget, refused by the walk or refused on its size is a 400 (`could not read the PDF: it was too costly to read (…)`, `could not read the PDF: <refusal>`, `could not read the PDF: it was too costly to read (the reader's result was too large)`), never a 500; a reader process that fails on its own — an import error, a bug — is a 500, the server's fault rather than the file's, with its exit status in the log. The heading is looked for one line at a time (a fullmatch per line, with possessive quantifiers and only space and tab as whitespace), so a hostile run of spaces or `|` costs its length: a pattern run over the joined text took 33 seconds on 100,000 spaces, in the request thread, after the process had returned the pages. The text the model reads starts at the report's **last** reference-list heading (a line reading `References`, `Bibliography`, `Works Cited`, `Reference list` or `Literature cited`, optionally numbered, optionally followed by a page number or set beside a `|`-separated running title, as `2024 Global Hunger Index | Bibliography  51`; a mid-sentence mention, a title glued to a sentence's end or a contents leader is not a heading) — or, walking back while each earlier heading sits at most 2 pages (`HEADING_RUN_PAGES`) before the one after it, at the earliest heading of that run, since a bibliography that spans several pages prints its heading on each page (or every other page) as a running header, while a chapter-end list or a contents-page mention farther back stays excluded; the distance is counted in pages, not characters, so it does not depend on how dense a page is. A plain heading anchors the text at the heading word; a running-header form (a page number after the heading, or the `|` title) names its page, so the text starts at that page's start — pypdf emits a footer after the page's entries, and anchoring at it would drop the first page of the list. The text is capped at 65,000 characters (`REFERENCE_MAX_CHARS`; the largest example bibliography, 11 pages, is 61,616); a report without such a heading gets the last 65,000 characters of the document instead. A file with no extractable text at all (a scanned PDF) is answered without a model call, so no bibliography can be invented for it. The model that reads the printed entries into fields is `references_model` (`AUTHORAI_REFERENCES_MODEL`, default `claude-haiku-4-5`; see [configuration.md](configuration.md)). It reads the text in **parts** of at most 12,000 characters (`REFERENCE_CHUNK_CHARS`), cut at a blank line or, failing one in the second half of the part, at a line break — never inside a line, except that a line longer than a whole part (text that lost its line breaks) is cut at its last space or at the cap — one structured call per part and four parts at a time, because one call's output budget (16,000 tokens) cannot carry a whole long list; each part's prompt says which part it is and that a part may begin or end in the middle of an entry, and such a fragment is listed as printed, never completed. The parts' answers are joined in order. The largest real list, the Drought report's ~224 entries in ~62,000 characters, is 6 calls, ~16k input and ~25k output tokens, roughly $0.15; a typical report's list costs under a cent. The cap of 300 references is applied after every part has answered, not as an early stop, so the true worst case is a 65,000-character list of very short entries (~64 characters, ~1,000 entries): still 6 calls, ~55k output tokens, about $0.30 at Haiku's $1/$5 per million tokens, with ~700 entries then discarded. A part whose answer the SDK cannot read (invalid JSON — live, a string the model looped on until the token limit) is asked once more like an incomplete one, and a part unreadable twice fails the scan with a 502 the dialog can show; a part whose call fails otherwise fails the scan (500) — either way the other parts' entries are not returned in its place, since a list with a silent hole would report cited works as missing from the user's sources when they were merely unread.

**Known limitations.**

- A scan the dialog abandons — the user picks another report, or closes the dialog, while a scan is running — is not cancelled on the server: its model calls complete and the answer is discarded. The cost is bounded by the caps above (at most 6 calls for the largest list), nothing is stored, and at most 2 scans run at once (`SCAN_CONCURRENCY`): a third — re-picking reports while their scans drain — is answered 429 at once, before any reader process or model call, rather than stacking reader processes on the server; the dialog shows the message beside its Try again control.
- As in `POST /api/runs`, the per-file cap is checked from the spooled part's size, so a report between the 50 MB per-file cap and the 220 MB whole-request cap is received and spooled to disk before its 413 is answered — accepted, since the scan then reads nothing and stores nothing.
...
`text_source` says where the text came from: `heading` (from the reference-list heading, as above), `tail` (no heading; the document's end) or `none` (no text at all; `references` is then empty). At most 300 references are returned (`MAX_REFERENCES`); a longer model answer is cut after every part has answered, with a warning in the server log and the count in `limits.references_dropped`.

`limits` says what the caps took, so the dialog can say a list was read in part rather than present the works it never read as absent from the user's sources: `text_truncated` is `true` when the closing text was cut at 65,000 characters (`REFERENCE_MAX_CHARS`), from the heading forward or, for a tail, from the document's end back; `references_dropped` counts the rows not returned — those past the cap of 300, and the blank rows described below; `possibly_incomplete` is `true` when a part's answer still looked incomplete after its one retry. Six identical live calls on a 37-entry list answered 37 / 1 / 37 / 37 / 1 / 1 references (the model closing the list after one short object, or putting the whole list into one object's text), so besides a temperature of 0 and a schema with no verbatim field (see below) the server holds each part's answer against a cheap count of the lines shaped like an entry's start (`entry_starts`: an author-year line, a numbered line with a comma or a year, a surname-comma-initial line; calibrated on the example reports): an answer with fewer than half as many objects, where the text shows at least 4 such lines (`ENTRY_STARTS_FLOOR`), or with any string field over 1,000 characters before the cuts (`DEGENERATE_FIELD_CHARS`: entries merged into one object's text, or a field the model looped on; the longest real entry measured is 759) is asked for once more with the same prompt, the answer that passes the check is kept — else the one with more references — and one that still looks incomplete sets this flag, with a warning in the server log giving both counts — the dialog then says the scan may have missed some entries and offers to try again. All three are `false`/`0` for a list read whole.

Each reference carries what its entry **prints** — `title`, `authors`, `year`, `doi` (no URL prefix) and `url` are `null` or empty when the entry does not print them, and the model is told never to supply a DOI from memory — plus `label`, a short key for the entry as printed — the first author's surname or the organisation, plus the year, as `Adler 2011` — cut to 40 characters (`LABEL_MAX_CHARS`) and `null` when the model prints none, which the dialog shows when there is no title. The anchor that ties a row to the page is title + year + identifier, verified in code (the lookup keys on the DOI; the dialog matches the title against the user's sources), and no field asks for verbatim text: an earlier required `entry` field, the entry's first 160 characters exactly as printed, made the model return one entry for a 37-entry list at temperature 0 and sometimes loop on one character inside it until the token limit — measured live, three calls per variant, 1 / 1 / 1 with the verbatim field against 37 / 37 / 37 with the label. A `title` is cut to 500 characters (`TITLE_MAX_CHARS`), `authors` to 50 names (`MAX_AUTHORS`) of at most 200 characters each (`AUTHOR_MAX_CHARS`), in code after parsing like the label, since the schema cannot enforce a length and these fields are shown and matched. A `url` over 2048 (`URL_FIELD_MAX_CHARS`, the link gate's own length) or a `doi` over 300 (`DOI_FIELD_MAX_CHARS`; the lookup refuses one over 256 before any request) is dropped from its row — `null`, with a warning in the server log — and never cut: an identifier cut names something else, and a cut address of exactly the gate's length would be offered as a link the page never printed. A row with only an address or a DOI is labelled by it, and a row left with nothing that names the work is dropped like any other. A reference is kept only when it names a title, DOI or address the dialog can show and check; a row with a label alone, or with authors and a year alone, is not a reference the user can act on and is dropped, with a warning in the server log that counts the drops. `retrievability` and `suggested_url` come from Unpaywall, asked by DOI:
...
| `unknown` | No DOI printed, the DOI was not found, the record did not say or was of the wrong shape, no lookup was made, or the registry's address failed the link gate | `null`, **except** for an entry with no DOI that prints its own address: that address is offered as addable but unchecked — the dialog labels it "printed in the entry, not checked" |

Every `suggested_url` has passed the syntax gate a pasted link passes (`http`/`https`, a valid host, no credentials, ≤ 2048 characters; the `#fragment` dropped, scheme and host lowercased) and two checks more. The first is the refusal `POST /api/runs` makes of a YouTube address (`youtube.com`, `youtu.be`, `youtube-nocookie.com` and their `www.`, `m.` and `music.` hosts): a suggested address is meant to be added as a link, so one the dialog would refuse is never suggested — every refusal the dialog's own link check makes, the scan makes first. The second is the refusal the fetcher would make without a network: a literal IP host must be a public address (loopback, private, link-local, carrier-grade NAT, multicast and reserved ranges are refused, IPv4 wrapped in IPv6 included), and `localhost` or any name under it is refused. A name is not resolved — the scan never connects to an address, and never looks one up — so a name that resolves to a private address is offered here and refused at ingest by the fetch gate, which does resolve it. An address that fails is dropped with a warning in the server log, and the record's next address is tried. A DOI is looked up only when it is DOI-shaped (`10.NNNN/…`) and at most 256 characters (`DOI_MAX_CHARS`); anything else is `unknown` without a request. A DOI several entries print is looked up once, and the verdict reaches every entry that prints it. A registry record of the wrong shape — a `best_oa_location` that is not an object, an address that is not a string — is `unknown` with a warning in the server log, never a failure.
