...
| References returned by a scan | 300 (`MAX_REFERENCES`); a longer model answer is cut in code, with a warning in the log. Each `entry` is at most 160 characters (`ENTRY_PREFIX_CHARS`) | In `POST /api/references/scan` |
...
`text_source` says where the text came from: `heading` (from the reference-list heading, as above), `tail` (no heading; the document's end) or `none` (no text at all; `references` is then empty). At most 300 references are returned (`MAX_REFERENCES`); a longer model answer is cut, with a warning in the server log.
