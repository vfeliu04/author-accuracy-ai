...
- A scan the dialog abandons — the user picks another report, or closes the dialog, while a scan is running — is not cancelled on the server: its model calls complete and the answer is discarded. The cost is bounded by the caps above (at most 6 calls for the largest list), and nothing is stored.
