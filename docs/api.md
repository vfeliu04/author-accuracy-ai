...
| Time and memory to read the report PDF in a scan | 60 seconds of wall clock (`extract_timeout_seconds`, the budget a fetched web page gets), in a separate process stopped at the deadline, with a CPU limit of that budget plus 5 seconds and, where the platform enforces one (Linux; macOS refuses the call), a 1 GiB address-space cap (`READER_ADDRESS_SPACE_BYTES`). Before pypdf flattens the page tree the process walks it itself and refuses a declared page count over 10,000 (`PAGE_TREE_CEILING`), a walk of more than 10,000 nodes, or a node reached twice | In `POST /api/references/scan` → 400 `could not read the PDF: it was too costly to read (…)` or `could not read the PDF: <refusal>` |
