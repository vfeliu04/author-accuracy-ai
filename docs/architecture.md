...
Two judgments, split as elsewhere: code decides what the model sees, the model reads the printed entries into fields, and code bounds and checks the answer. `read_pages` takes the text of the last 600 pages (`REFERENCE_MAX_PAGES`) with pypdf (`strict=False`; an empty user password is tried on an encrypted file; any reader failure is one `ValueError`, the endpoint's 400) — pypdf rather than Docling because a layout pass is seconds per page and a bibliography needs no layout. `reference_text` starts at the **last** line that is a reference-list heading (`References`, `Bibliography`, `Works cited`, `Reference list` or `Literature cited`, optionally numbered or colon-terminated, nothing else on the line) — or, walking back while each earlier heading sits at most 12,000 characters (`HEADING_RUN_GAP`, two dense pages: the running headers of the Drought report's 11-page list sit 5,072 to 6,064 characters apart) before the one after it, at the earliest heading of that run, so a multi-page bibliography whose running header repeats the heading is read whole while a chapter-end list or a contents-page mention farther back stays out; the window is page-sized, not cap-sized, so the cap bounds what the model reads and not what counts as one list — and keeps 60,000 characters (`REFERENCE_MAX_CHARS`: the largest real bibliography measured spans ~57,000 characters between its first and last running header, about 15k model input tokens); a report without one gets its last 60,000 characters; no text at all means no model call, so a scanned PDF cannot yield an invented bibliography. `extract_references` is a structured call on `references_model` (`claude-haiku-4-5`) whose prompt, `REFERENCES_SYSTEM`, is the mirror image of `credibility.METADATA_SYSTEM` (which tells the model to ignore reference lists): every entry describes a work the report cites, never the report; a DOI or address only when the entry prints it, never from memory; `entry` is the printed text verbatim, shown when there is no title. The answer is capped at 150 references (`MAX_REFERENCES`; the prompt tells the model the list may be long and to list every entry) in code, not in the schema — structured outputs may not honour a `max_length`, and a client-side validation failure would 500 the scan instead of trimming it.
