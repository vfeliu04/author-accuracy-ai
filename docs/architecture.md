...
| `references.py` | The upload dialog's reference scan: pypdf text of a report's closing pages, its printed reference list as fields (structured LLM call), Unpaywall retrievability — a pre-upload aid that persists nothing |
...
trafilatura runs in fast mode, and its full extraction cascade runs only when fast mode finds the page thin. Sections are split at the heading elements of the extracted tree, never by parsing Markdown; content before the first heading gets an empty title, and empty sections are dropped. Each section is written with trafilatura's own Markdown writer, so tables stay inline as pipe tables. Fewer than `MIN_BODY_CHARS` (250) characters of section text — headings not counted — raises `ThinPageError` naming the URL: `<url> has no readable article text (<n> characters extracted, at least 250 needed) — JavaScript-only pages are not supported`. The document's title is the declared title, else the first heading. The ingest step then caps how much of the page is kept: sections past `AUTHORAI_WEB_MAX_CHARS` (200,000 characters) are dropped before the page is stored, with a warning naming the page and the characters dropped — whole sections, except a first section that alone exceeds the cap, which is cut at its last line break. The fetch bounds a page's markup, nothing bounds what the reader gets out of it, and a document's sections are chunked and embedded in one list. A cut is **recorded with the page**, not only logged: the stored snapshot's provenance gains `truncated: {kept_chars, dropped_chars}` (absent when the page was read whole), which survives into `documents.metadata` and out through the report's source rows, where the source list marks the row *Read in part* with the two numbers on hover and the chat context names the same two numbers (`READ IN PART`, so the model does not answer from the head of a page as though it had the page). A report scored against the head of a page must not read like one scored against the whole page.
...
**Decoding.** When the fetched body is not valid UTF-8, does not start with a byte-order mark (UTF-8 or UTF-16), and the HTTP `Content-Type` names a text encoding, the ingest step decodes the body with that charset (`iso-8859-1` and `ascii` as windows-1252, as browsers do). A label Python does not know, one that names no text encoding (`base64`, `bz2_codec`), or a codec that refuses every byte (`undefined`) is ignored, and so is the header whenever the body starts with a byte-order mark. Otherwise `extract_web` decodes the bytes itself: a UTF-16 or UTF-8 byte-order mark first, over any `<meta>` charset as well as the header (a sequence the marked encoding cannot decode becomes U+FFFD rather than sending the whole page to another encoding), then UTF-8 whenever the bytes are valid UTF-8, then a `<meta>` charset declared in the first 4096 bytes (ISO-8859-1 and ASCII labels as windows-1252, a UTF-16 label as UTF-8; a label that is unknown, names no text encoding, or refuses every byte fails as `<url> declares an unknown charset '<label>'`, the label cut to 200 characters like every other value a failure quotes). Bytes that fit none of these fail loudly, naming the URL — a guessed encoding would corrupt text that is later quoted as evidence. Whichever decides — the HTTP charset, a byte-order mark, or a `<meta>` charset — a byte sequence that encoding cannot decode becomes U+FFFD with a warning in the server log naming the page where the fetch ended and how many sequences were replaced: `<url>: <n> undecodable byte sequence(s) replaced (HTTP charset '<label>')`, with `UTF-8 byte-order mark`, `UTF-16 byte-order mark`, or `declared charset '<label>'` as the source on the other paths.
...
| Reference scan | `claude-haiku-4-5` |
...
- **Credibility** — per source: Haiku extracts bibliographic metadata from the opening chunks, except that a web page whose markup declared authors, a publisher, a date, or a DOI is scored from that declaration with no model call. Verification assigns a tier (`VERIFIED_DOI` / `VERIFIED_TITLE` / `VERIFIED_ISBN` / `MATCHED_RECORD` / `METADATA_ONLY` / `NONE`) through Crossref and, for ISBNs, Open Library or Google Books — every external hit must be corroborated by the document's own title, author or publisher before it earns a tier or is merged, since a DOI or ISBN the document merely prints is no proof the record found is its own, and for a **fetched source** — a page, or a PDF a pasted link served, since the rule follows the fetch and not the media type — the record behind **any** verified tier must additionally name the address it was fetched from (the record's landing page IS that address — host and path — or that address carries the identifier; a page is compared by the address it landed on after redirects, a fetched PDF by the address the link was pasted at, the only one its fetch records), a fetched source with no usable address earning no verified tier at all; a refused candidate is never returned, so the matched work's publisher, date and title are never merged, but the refusal itself is scored as `MATCHED_RECORD` (10 of 20) rather than the `METADATA_ONLY` floor (5), since a registered work matching this document at another address is not the same finding as no record at all (a source that could not be PLACED — no usable address — stays at the floor instead: nothing was ever compared for it, so there is no such finding to report) — a page writes its corroborating fields in the same invisible tags as its DOI, an uploaded file is the file the operator meant, so nothing the app fetched can verify itself, on any path; a web page gets the Crossref title search only when it carries `citation_*` tags. Code sums component points (completeness, word-boundary publisher authority, recency, verification — **no floors**). An `image` source is never scored: it is listed under `excluded` with its usage and never averaged. Aggregated as a usage-weighted mean, where usage counts quote-verified verdicts citing each source (SUPPORTED **and** CONTRADICTED — a contradicting source is doing its job). Crossref 404 is an answer; 429/5xx retries then **raises** (a throttled Crossref must not silently downgrade tiers).
...
## Reference scan (`references.py`)

A pre-upload aid, outside the pipeline: when the user picks the report PDF, the upload dialog posts it to `POST /api/references/scan` and shows **Cited by the report, not among your sources** — the works the report's own reference list cites, minus those the dialog already holds, with the ones that have a free copy online offered as links. It persists nothing (the handler has no database dependency, so it cannot create a run, upload or job row; the spooled part is read in place and no file lands under `uploads_dir`), fetches nothing (a suggested address passes `fetch.validate_source_url`, the gate a pasted link passes, and then `fetch.is_public_address` when its host is a literal IP, with `localhost` refused by name; it is never resolved or visited — a name pointing at private space is refused at ingest), and caches nothing.

Two judgments, split as elsewhere: code decides what the model sees, the model reads the printed entries into fields, and code bounds and checks the answer. `read_pages` takes the text of the last 600 pages (`REFERENCE_MAX_PAGES`) with pypdf (`strict=False`; an empty user password is tried on an encrypted file; any reader failure is one `ValueError`, the endpoint's 400) — pypdf rather than Docling because a layout pass is seconds per page and a bibliography needs no layout. `reference_text` starts at the **last** line that is a reference-list heading (`References`, `Bibliography`, `Works cited`, `Reference list` or `Literature cited`, optionally numbered or colon-terminated, nothing else on the line) — or, walking back while each earlier heading sits at most 2 pages (`HEADING_RUN_PAGES`) before the one after it, at the earliest heading of that run, so a multi-page bibliography whose running header repeats the heading on each page, or on alternate pages, is read whole while a chapter-end list or a contents-page mention farther back stays out. Each heading's page comes from the per-page offsets of the joined text, and the walk chains — each heading is judged against the one before it, not against the last — so a run of any length joins; the distance is in pages, not characters, so it is independent of page density and of the cap, which bounds what the model reads and not what counts as one list. The slice keeps 65,000 characters (`REFERENCE_MAX_CHARS`: the largest real bibliography measured, the Drought report's 11-page list, runs 61,616 characters from its first running header to the end of its last page — an earlier heading-to-heading measurement, 57,173, left out the last page); a report without a heading gets its last 65,000 characters; no text at all means no model call, so a scanned PDF cannot yield an invented bibliography. `extract_references` reads that text in **chunks**: `split_reference_text` cuts it into pieces of at most 12,000 characters (`REFERENCE_CHUNK_CHARS`), at the last blank line in the second half of the window or else the last line break, never inside a line (a line longer than a chunk — text that lost its line breaks — is cut at its last space, or at the cap), and each chunk is its own structured call on `references_model` (`claude-haiku-4-5`), run 4 at a time (`LOOKUP_WORKERS`) through a thread pool whose `map` keeps chunk order, raises the first failure and cancels the chunks not yet started. One call cannot carry a whole long list: `PARSE_MAX_TOKENS` is 16,000 and the Drought list's ~224 entries with a verbatim `entry` would need 20-32k output tokens, so the single call the scan first made would have been cut off and 500ed on the very report the caps were sized for. The largest real list, the Drought report's ~224 entries in ~62,000 characters, is 6 chunks, ~16k input tokens and ~25k output tokens across the pool, roughly $0.15; a typical report is under a cent. The 300-reference cap is applied after every chunk has answered, not as an early stop, so the true worst case is a 65,000-character list of very short entries (~64 characters, ~1,000 entries): still 6 calls, ~55k output tokens, about $0.30 at Haiku's $1/$5 per million tokens, with ~700 entries then discarded — accepted rather than read in waves with a check between them, which would hold the later chunks until every call of the wave before returned and add latency to every long list to save a fraction of a dollar on lists the scan cuts to 300 anyway. A failed chunk fails the scan (500, as any model failure) rather than returning the other chunks' entries: a bibliography with a silent hole would report cited works as missing from the user's sources when they were merely unread. The prompt, `REFERENCES_SYSTEM`, is the mirror image of `credibility.METADATA_SYSTEM` (which tells the model to ignore reference lists): every entry describes a work the report cites, never the report; a DOI or address only when the entry prints it, never from memory; `entry` is the first 160 characters of the printed text (`ENTRY_PREFIX_CHARS`, asked for in the prompt and cut in code after parsing), shown when there is no title; a chunk's prompt says it is part N of M and that a part may begin or end mid-entry, and such a fragment is listed with what is printed, never completed. The chunks' answers are concatenated in chunk order and capped at 300 references (`MAX_REFERENCES`; the prompt tells the model the list may be long and to list every entry) in code, once every chunk has answered, not in the schema — structured outputs may not honour a `max_length`, and a client-side validation failure would 500 the scan instead of trimming it.

Retrievability comes from Unpaywall (`UnpaywallClient`), keyed by the printed DOI after `credibility.clean_doi`, under the registry-GET policy Crossref already follows (`get_json_with_retries`, made public for it: a 404 — an HTML page, live — is an answer, `None`; 429/5xx and transport failures retry twice on a 10-second timeout, then raise). Lookups run four at a time in a thread pool; the first registry failure stops the queued ones, and the endpoint still answers 200, with `lookup.status` `unavailable` and the verdicts reached so far, because the citation list is useful without retrievability. `is_oa: false` is `paywalled`, never offered; an open work is `pdf` when `url_for_pdf` is set, else `landing` (for many genuinely open works Unpaywall's `url_for_pdf` is null and `url` is the landing page); anything else is `unknown`. A DOI-less entry that prints its own address gets it as `suggested_url` with retrievability `unknown` — offered unticked and labelled *printed in the entry, not checked*. Unpaywall requires a contact email, so `AUTHORAI_CROSSREF_MAILTO` (one operator, one address) also gates the lookup: unset, no request is made and `lookup.status` is `unconfigured`.
...
| `POST /api/references/scan` → 200 | The report PDF's own reference list, each cited work with whether Unpaywall knows a free copy; no database dependency, so it cannot create a row, and nothing lands under `uploads_dir` |
...
- **Cited works in the dialog.** Picking the report starts `useReferenceScan`, a `useQuery` keyed on the file's name, size and `lastModified` (`retry: false`, `staleTime: Infinity`, `gcTime` 60 s) whose `queryFn` hands TanStack's `signal` to the request, so swapping the report or closing the dialog aborts a scan in flight — the codebase's first abort path, with no controller of its own. Which cited works are still missing is computed client-side on every render (`lib/references.ts`, `alreadyAdded`), because the report is picked before its sources exist and a server verdict at scan time would be stale for exactly the case the checklist is for. Four rules, first hit wins: a link carrying the DOI (`doiInUrl`, `clean_doi`'s shape as a whole path segment, so doi.org and publisher URLs both count); a link to the printed or suggested address (judged by `checkLink`'s own duplicate rule); a file named after the title (mirror of `credibility._normalize_title`, whole words, at least four of them on the contained side); a file named after a family name and the year. Rows with a found copy (`pdf`, `landing`) are ticked by default, a printed-but-unchecked address is not; **Add N links** is the only writer: each ticked address goes through `checkLink` against the growing list (duplicates skipped) and stops at the 20-source cap with *Added N of M — at most 20 sources per verification*. `canSubmit` never depends on the scan, so a scan in flight or failed never blocks **Verify**.
