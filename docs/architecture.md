...
| authors | `citation_author` (all) > JSON-LD personal author names > `author` meta (one, unless the page declares that name as an organization: a JSON-LD Organization author, a JSON-LD publisher, `citation_publisher`, or `og:site_name`, compared case-insensitively) |
...
| `DELETE /api/runs/{id}` → 204 | Delete a run's rows, the files the app stored for it (for a link, every file it left), and figure images — never a file outside `uploads_dir` or one another upload still names; 409 while its job is queued or running |
