...
| `documents` | One row per ingested PDF: `run_id`, kind (`SOURCE`/`REPORT`), title, metadata JSON (report sections), `embedding_model` (which model made its vectors — the dedup donor filter) |
