...
- **An institutional or publisher PDF fetched from a link**, whose registered DOI is real but whose address is not the registry's landing page and does not carry the DOI — a direct `…/files/report.pdf`, a mirror, a CDN copy. It drops from `VERIFIED_DOI` to `METADATA_ONLY`, because the rule follows the fetch and not the media type. The same PDF **uploaded** as a file is not gated at all: the operator chose it.
